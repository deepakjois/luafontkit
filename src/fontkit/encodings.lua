...
local ENCODINGS = {
  ...
  {'symbol', 'utf16be', 'shift-jis', 'gb18030', 'big5', 'wansung', 'johab', nil, nil, nil, 'utf16be'}
}

-- Overrides for Mac scripts by language id.
-- See http://unicode.org/Public/MAPPINGS/VENDORS/APPLE/Readme.txt
local MAC_LANGUAGE_ENCODINGS = {
  [15] = 'maciceland',
  [17] = 'macturkish',
  [18] = 'maccroatian',
  [24] = 'maccenteuro',
...
local function getEncoding(platformID, encodingID, languageID)
  if not languageID then languageID = 0 end
  if platformID == 1 and MAC_LANGUAGE_ENCODINGS[languageID] then
...
return {
  ...
  ENCODINGS = ENCODINGS,
  MAC_LANGUAGE_ENCODINGS = MAC_LANGUAGE_ENCODINGS,
  LANGUAGES = LANGUAGES
}
